    ...
    int minPathSum(vector<vector<int>>& grid) {
        ...
        */
        ...
        for(int currRow = m-1;currRow>=0;currRow--){
            vector<int>temp(n,0);
            for(int currCol = n-1; currCol>=0; currCol--){
                if(currRow == grid.size()-1 && currCol == grid[0].size()-1)temp[currCol] = grid[currRow][currCol];
                else if(currRow == grid.size()-1){
                    temp[currCol] = grid[currRow][currCol] + temp[currCol+1];
                }else if(currCol == n-1){
                    temp[currCol] = grid[currRow][currCol] + dp[currCol];
                }
                else{
                    temp[currCol] = grid[currRow][currCol] + min(temp[currCol+1],dp[currCol]);
                }
            }
            dp = temp;
        }
